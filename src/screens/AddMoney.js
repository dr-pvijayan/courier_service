...
export default function AddMoneyScreen(props) {
  ...
  const newData = ({ item, index }) => {
    return (
      <TouchableOpacity style={[styles.boxView, { backgroundColor: item.selected ? colors.GREEN.light : colors.GREY.primary }]} onPress={() => { quckAdd(index); }}><Text style={styles.quckMoneyText, { color: item.selected ? colors.WHITE : colors.BLACK }} >{settings.symbol}{item.amount}</Text></TouchableOpacity>
    )
  }
